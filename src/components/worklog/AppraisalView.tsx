...
export default function AppraisalView() {
  ...
  const metrics = useMemo(() => {
    ...
    const leaveDays = monthSessions.filter(s => s.dayStatus === 'annual_leave' || s.dayStatus === 'half_day_leave').reduce((acc, s) => acc + (s.dayStatus === 'half_day_leave' ? 0.5 : 1), 0);
    ...
    <div className="flex flex-col gap-6 h-full px-2 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20" dir="rtl">
      <header className="flex flex-col gap-1 mb-2 mt-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold">{t('t_auto_172')}</h2>
            <p className="text-muted-foreground text-sm">{t('t_auto_173')} {format(now, 'MMMM yyyy', {locale: ar})}</p>
          </div>
        </div>
      </header>
      ...
      <Card className="p-6 bg-card border-white/5 rounded-[2rem] space-y-4 shadow-lg relative overflow-hidden">
         ...
         <h3 className="font-bold flex items-center gap-2 relative z-10"><Award className="w-5 h-5 text-amber-500"/> {t('t_auto_174')}</h3>
         <p className="text-sm leading-loose text-muted-foreground relative z-10">
            {t('t_auto_175')} <b className="text-foreground">{metrics.totalExtraHrs}</b> {t('t_auto_176')} <b className="text-foreground">{metrics.totalRestDaysWork}</b> {t('t_auto_177')}
                           </p>
         ...
         <div className="grid grid-cols-2 gap-3 mt-4 relative z-10">
             <div className="bg-primary/5 p-4 rounded-2xl flex flex-col items-center justify-center gap-1 text-center border border-primary/10">
                ...
                <span className="text-[10px] md:text-xs text-muted-foreground font-bold">{t('t_auto_178')}</span>
             </div>
             <div className="bg-red-500/5 p-4 rounded-2xl flex flex-col items-center justify-center gap-1 text-center border border-red-500/10">
                ...
                <span className="text-[10px] md:text-xs text-muted-foreground font-bold">{t('t_auto_179')}</span>
             </div>
         </div>
      ...
      <div className="bg-gradient-to-l from-secondary/40 to-secondary/10 p-5 rounded-3xl flex flex-col gap-2 border border-white/5 shadow-inner">
         <h4 className="font-bold text-sm text-foreground flex items-center gap-2"><Zap className="w-4 h-4 text-emerald-500" /> {t('t_auto_181')}</h4>
         <p className="text-xs text-muted-foreground leading-relaxed mt-1">
            {t('t_auto_182')} {format(now, 'MMM', {locale: ar})}{t('t_auto_183')}
                           </p>
      </div>
    </div>
  );
